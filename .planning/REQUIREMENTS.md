...
- [x] **UPD-01**: Preserve the existing GitHub-API-only network allowlisting and response validation in the update checker — don't regress this — **live-verified 2026-07-08 (Phase 4 Plan 04):** on a real macOS Photoshop install, `checkForUpdates()` completed a real round-trip to `api.github.com` with no console error, confirming the allowlisting/validation logic behaves correctly outside of Jest's mocked-fetch tests too.
- [x] **UPD-02**: Preserve the existing manual "update available → click → browser download" flow — don't regress this — **live-verified 2026-07-08 (Phase 4 Plan 04):** clicking Download in a real Photoshop panel opened the system browser to the correct GitHub release page, confirmed directly by the user.
- [x] **UPD-03**: Reconnect the update checker end-to-end (`checkForUpdates()`/`UpdateBanner` are currently disconnected dead code per the codebase's own CONCERNS.md) so the manual update flow actually works, not just exists in source — **new constraint found 2026-07-06:** Phase 1 empirically confirmed that declaring `requiredPermissions.network` in `manifest.json` triggers Creative Cloud Desktop's admin-password prompt on install/update for non-Marketplace plugins (removing it avoids the prompt). Phase 1 removed this permission since the update checker wasn't wired up yet. Re-adding it here to make network calls work will likely reintroduce the admin-password prompt — this needs a conscious decision during Phase 4 planning (accept the prompt, find an alternative mechanism, or reconsider scope), not a silent re-add. Full detail in 01-RESEARCH.md's second follow-up addendum. **Live-verified 2026-07-08 (Phase 4 Plan 04) — contradictory result, genuinely unresolved:** the reconnected checker was confirmed firing live end-to-end (banner appeared, no console error). However, the admin-password prompt anticipated above **did not appear** on this live install, despite the credential-caching confound being explicitly ruled out by the user. This directly contradicts Phase 1's original A/B test finding rather than confirming it — two trials now disagree. Not resolved either direction; Phase 5 must not assert unconditionally that the prompt will or won't appear. See `04-04-SUMMARY.md` for full detail and the recommended controlled re-test.
